...
if __name__ == '__main__':
    ...
    with open('/Users/erdemisbilen/TFvenv/input/bad-words.csv') as bf:
        bad_words = bf.readlines()
    bad_words = [word.strip() for word in bad_words]    
    
    # fetch english words dictionary
    with open('/Users/erdemisbilen/TFvenv/input/20k.txt') as f:
        words = f.readlines()
    ...
    sample_text = ' In general, low-level image features are minor details of the image, sch as lines, edges, corers or dots. High-level featres are built on top of low-level features to detect ojects and larger sapes in the image. We can extract both type of features uing conlutional neural networks: the first couple of convolutional layers will learn filters for finding low level features while the later layers will learn to recognize common shapes and objects.'
