  ...
  for token in my_doc:
    if token.is_punct != True and token.is_alpha == True :
      ...
      if (token.pos_ == "VERB" or token.pos_ == "ADJ" or token.pos_ == "ADV") and (random.choice(RANDOM_LIST)==1 or missing_word==""):
        ...
        most_similar = nlp.vocab.vectors.most_similar(w_vector.reshape(1,300), n=25)
...
matcher_possesive_pronoun.add('POSSESIVE_PRONOUN', None, possesive_pronoun_rule_1)

matcher_prepositions_sub_conj.add('PREPOSITION', None, prepositions_sub_conj_rule_1)
...
with open(Settings.ARTICLE_SENTENCES_PROCESSED_JSON_FILE, "w") as text_file:
  ...
  for line in lines:   
    ...
    if sentence_length < EASY_MAX_SENTENCE_LENGTH and sentence_length > EASY_MIN_SENTENCE_LENGTH :
      ...
      missing_word, missing_word_pos, option1, option2, option3 = define_missing_word(my_doc)
      ...
      possesive_pronoun = write_linguistic_features(matches_possesive_pronoun)
      pos_pro = AddOptions("POSSESIVE_PRONOUN", possesive_pronoun)
      possesive_pronoun.insert(0,pos_pro.add_options()) 
      
      preposition = write_linguistic_features(matches_prepositions_sub_conj)
      pp = AddOptions("PREPOSITION", preposition)
      preposition.insert(0,pp.add_options())
      
      adverb_particle = write_linguistic_features(matches_adverb_particle)
